...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!supabase) {
      console.error('Confirm-email route: SUPABASE_SERVICE_ROLE_KEY is not configured')
    ...
    if (!process.env.RESEND_API_KEY) {
      console.error('Confirm-email route: RESEND_API_KEY is not configured')
      return NextResponse.json({ error: 'RESEND_API_KEY is not configured' }, { status: 500 })
    }

    if (!confirmationLink) {
      console.error('Confirm-email route: confirmation link is missing')
      return NextResponse.json({ error: 'Confirmation link is missing' }, { status: 500 })
    }
