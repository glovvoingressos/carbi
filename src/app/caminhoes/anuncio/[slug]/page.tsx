...
import { getPublicListingBySlug, getRelatedListings, getListingVehicleId, getSellerInfo } from '@/lib/marketplace-server'
...
export default async function TruckDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const [related, sellerInfo] = await Promise.all([getRelatedListings({ brand: listing.brand, model: listing.model, yearModel: listing.year_model, excludeId: listing.id, limit: 6 }), getSellerInfo(listing.user_id)])
  void getListingVehicleId
