  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white border border-[var(--color-border-strong)] rounded-xl px-5 py-4 flex items-center justify-between">
        <div><div className="text-2xl font-heading font-bold text-[var(--color-text-primary)]">{(listing.view_count || 0).toLocaleString('pt-BR')}</div><div className="text-[10px] font-bold text-[var(--color-text-tertiary)] uppercase tracking-wider">visualizações</div></div>
      ...
      <div className="bg-white border border-[var(--color-border-strong)] rounded-xl p-5 space-y-3">
        ...
        <div><label className="text-[10px] font-bold uppercase tracking-wider text-[var(--color-text-disabled)] ml-1 mb-1 block">Valor (R$)</label>
          <input className={`${ic('price')} text-xl`} value={formData.price || ''} onChange={(e) => update('price', e.target.value)} />
