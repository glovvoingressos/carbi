...
function AvatarSection({ avatarUrl, fullName, email, userId, onAvatarChange, uploading, onUploadingChange }: {
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm">
      <h3 className="text-sm font-semibold text-gray-900 mb-5">Foto de perfil</h3>
      <div className="flex items-center gap-6">
        <div className="relative group shrink-0">
          <div className="w-24 h-24 rounded-full overflow-hidden bg-blue-50 border-4 border-white shadow-lg">
            {avatarUrl
              ? <img src={avatarUrl} alt="Avatar" className="w-full h-full object-cover" />
              : <div className="w-full h-full flex items-center justify-center bg-blue-100"><User className="w-10 h-10 text-blue-500" strokeWidth={1.5} /></div>}
          </div>
        ...
        <div className="min-w-0 flex-1">
          <p className="text-lg font-semibold text-gray-900 truncate">{fullName || 'Seu nome'}</p>
          <p className="text-sm text-gray-500 truncate mt-1">{email}</p>
          <label htmlFor="avatar-upload" className="inline-flex items-center gap-2 mt-4 px-5 py-2.5 text-sm font-medium text-blue-600 bg-blue-50 rounded-xl hover:bg-blue-100 transition-colors cursor-pointer">
...
function PersonalInfo({ fullName, email, phone, onNameChange, onPhoneChange }: {
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm">
      <h3 className="text-sm font-semibold text-gray-900 mb-5">Dados pessoais</h3>
      <div className="space-y-5">
        <div>
          <label className="text-sm font-medium text-gray-700 mb-2 block">Nome completo</label>
          <input value={fullName} onChange={(e) => onNameChange(e.target.value)} placeholder="Seu nome" className="w-full h-12 px-4 rounded-xl bg-gray-50 border border-gray-200 text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100 transition-all" />
        </div>
        <div>
          <label className="text-sm font-medium text-gray-700 mb-2 block">E-mail</label>
          <input value={email} disabled className="w-full h-12 px-4 rounded-xl bg-gray-100 border border-gray-200 text-sm text-gray-500 cursor-not-allowed" />
        </div>
        <div>
          <label className="text-sm font-medium text-gray-700 mb-2 block">Telefone / WhatsApp</label>
          <input value={phone} onChange={(e) => onPhoneChange(formatPhone(e.target.value))} placeholder="(00) 00000-0000" inputMode="tel" maxLength={15} className="w-full h-12 px-4 rounded-xl bg-gray-50 border border-gray-200 text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100 transition-all" />
        </div>
      </div>
    </div>
  )
...
function SecuritySection({ userId, toast }: { userId: string; toast: ToastFn }) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm">
      <h3 className="text-sm font-semibold text-gray-900 mb-5">Segurança</h3>
      <button
        type="button"
        className="w-full flex items-center justify-between p-4 rounded-xl bg-gray-50 border border-gray-200 text-sm font-medium text-gray-700 hover:bg-gray-100 transition-colors"
            ...
            <div className="pt-5 space-y-4">
              <div className="relative">
                <input
                  ...
                  className="w-full h-12 px-4 pr-11 rounded-xl bg-gray-50 border border-gray-200 text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100 transition-all"
                />
                <button type="button" className="absolute right-3 top-1/2 -translate-y-1/2 p-1.5 text-gray-400 hover:text-gray-600 transition-colors" onClick={() => setShowNewPw(!showNewPw)}>
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full h-12 px-4 pr-11 rounded-xl bg-gray-50 border border-gray-200 text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100 transition-all"
                />
                <button type="button" className="absolute right-3 top-1/2 -translate-y-1/2 p-1.5 text-gray-400 hover:text-gray-600 transition-colors" onClick={() => setShowConfirmPw(!showConfirmPw)}>
                  {showConfirmPw ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
                </button>
              </div>
              {newPw && confirmPw && newPw !== confirmPw && (
                <p className="text-sm font-medium text-red-500">As senhas não coincidem.</p>
              )}
              <div className="flex gap-3 pt-2">
                <button
                  type="button"
                  className="flex items-center gap-2 px-6 py-3 rounded-xl bg-blue-600 text-white text-sm font-semibold hover:bg-blue-700 transition-colors disabled:opacity-40 shadow-sm"
                  onClick={changePw}
...
function DangerZone({ userId, toast }: { userId: string; toast: ToastFn }) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-red-200 p-6 shadow-sm">
      ...
      <p className="text-sm text-gray-500 mb-5">Excluir sua conta é permanente. Todos os anúncios, favoritos e dados serão removidos.</p>
      {!show ? (
        <button
          type="button"
          className="inline-flex items-center gap-2 px-5 py-3 rounded-xl border border-red-200 text-red-600 text-sm font-medium hover:bg-red-50 transition-colors"
        ...
        <div className="space-y-4">
          <input
            ...
            className="w-full h-12 px-4 rounded-xl bg-gray-50 border border-red-200 text-sm text-red-600 placeholder-gray-400 focus:outline-none focus:border-red-500 focus:ring-2 focus:ring-red-100 transition-all"
          />
          <div className="flex gap-3">
            <button
              type="button"
              className="flex items-center gap-2 px-6 py-3 rounded-xl bg-red-600 text-white text-sm font-semibold hover:bg-red-700 transition-colors disabled:opacity-40 shadow-sm"
  ...
  return (
    <section className="space-y-8">
      ...
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1.5">Atualize suas informações pessoais</p>
      </div>
      ...
      {/* Save button */}
      <div className="sticky bottom-28 lg:bottom-6 pt-6 pb-2">
        <button
          type="button"
